...
class Fishyscapes(torch.utils.data.Dataset):
    ...
    def __init__(self, split='Static', root="", transform=None):
        ...
        if split == 'Static':
            self.images = glob.glob(f'{self.root}/fs_static/images/*.jpg')
            self.targets = glob.glob(f'{self.root}/fs_static/labels_masks/*.png')
        elif split == 'LostAndFound':
            self.images = glob.glob(f'{self.root}/FS_LostFound_full/images/*.png')
            self.targets = glob.glob(f'{self.root}/FS_LostFound_full/labels_masks/*.png')
        else:
            raise ValueError()
