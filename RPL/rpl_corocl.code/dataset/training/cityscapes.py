...
class Cityscapes(torch.utils.data.Dataset):
    ...
    def __init__(self, root: str = "/path/to/you/root", split: str = "val", mode: str = "gtFine",
        ...
        for city in os.listdir(img_dir):
            for file_name in os.listdir(os.path.join(img_dir, city)):
                target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                             self._get_target_suffix(self.mode, target_type))
                self.images.append(os.path.join(img_dir, city, file_name))
                self.targets.append(os.path.join(target_dir, city, target_name))
                self.predictions.append(os.path.join(pred_dir, city, file_name.replace("_leftImg8bit", "")))
