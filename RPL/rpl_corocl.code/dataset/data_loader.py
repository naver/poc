...
from dataset.cityscapes_synth import CityscapesSynth
import numpy
...
class Preprocess(object):
    def __init__(self, config):
        ...
        self.use_id_coco = config.use_id_coco
    ...
    def __call__(self, city_img, city_gt, ood_img, ood_gt, anomaly_mix_or_not):
        ...
        if not self.use_id_coco: # If this is activated then COCO in distribution classes are not used for extra training.
            ood_img = city_mix_img.copy() # We copy the mixed CS image so that ID classes in `ood_img` are all original CS.
            ood_gt = city_mix_gt.copy()
            
            # We modify the labels so that they correspond to the original ood_gt labels (i.e. binary 0, 1)
            ood_gt[(ood_gt > 18) & (ood_gt != 255)] = 254 # OOD label
            ood_gt[(ood_gt <= 18)] = 0 # ID label
...
class PreprocessCityscapesSynth(object):
    def __init__(self, config):
        self.img_mean = config.image_mean
        self.img_std = config.image_std
        self.city_image_height = config.city_image_height
    ...
    def __call__(self, city_img, city_gt, ood_img, ood_gt, anomaly_mix_or_not):
        city_img, city_gt = self.inlier_transform(city_img, city_gt)
        ood_img, ood_gt = self.inlier_transform(ood_img, ood_gt)
        assert ood_img.shape == city_img.shape, \
            print("ood_img.shape {}, city_img.shape {}".format(
            ood_img.shape, city_img.shape
        ))
        ...
        if anomaly_mix_or_not:
            city_mix_img = ood_img.copy()
            city_mix_gt = ood_gt.copy()
            city_mix_gt[(city_mix_gt > 18) & (city_mix_gt != 255)] = 254 # OOD label
        else:
            city_mix_img = numpy.zeros_like(city_img)
            city_mix_gt = numpy.zeros_like(city_gt)
...
def get_mix_loader(engine, config, proba_factor=1., ood_source='coco'):
    if ood_source.lower() == 'coco':
    ...
    elif ood_source.lower() == 'synth':
        train_dataset = CityscapesSynth(split='train', preprocess=PreprocessCityscapesSynth(config),
                                          cs_root=config.city_root_path, cs_synth_root=config.cs_synth_root_path)
    else:
