...
def main(gpu, ngpus_per_node, config, args):
    ...
    for curr_epoch in range(engine.state.epoch, config.nepochs):
        train_loader, train_sampler = get_mix_loader(engine=engine, config=config, proba_factor=args.prob, ood_source=args.ood_source)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ood_source', default='coco', type=str, help="source of OOD data")
    parser.add_argument('--nepochs', default=None, type=int, help="Number of epochs")
    parser.add_argument('--use_id_coco', default=None, type=str, help="Whether to use ID COCO classes for training.")
    ...
    if args.use_id_coco is not None:
        config.use_id_coco = args.use_id_coco == 'True'
        
    config.experiment_name += f'_p{args.prob}_e{config.nepochs}_{args.ood_source}_use_id_coco_{config.use_id_coco}_coco_classes'
    config.saved_dir += f'_p{args.prob}_e{config.nepochs}_{args.ood_source}_use_id_coco_{config.use_id_coco}_coco_classes'
