...
def valid_anomaly(model, engine, test_set, data_name=None, iteration=None, my_wandb=None, logger=None,
                  upload_img_num=4, measure_way="energy", anomaly_save_path=None):
    if engine.local_rank <= 0:
        logger.info("validating {} dataset with {} ...".format(data_name, measure_way))
    # dir_name = "ours"
    ...
    with torch.no_grad():
        for idx in tbar:
            ...
            if anomaly_save_path is not None:
                filename = f'{anomaly_save_path}/{data_name}/' + f'{idx}'.zfill(4) + '.npy'
                if not os.path.exists(f'{anomaly_save_path}/{data_name}/'):
                    os.makedirs(f'{anomaly_save_path}/{data_name}/')
                np.save(filename, visual.numpy())
