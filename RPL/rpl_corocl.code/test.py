...
from dataset.validation.cs_synth_ood import CityscapesSynth
from dataset.validation.idd_synth_ood import IDDSynth
from dataset.validation.acdc_synth_ood import ACDCSynth
...
def get_anomaly_detector(ckpt_path):
    ...
    model = Network(config.num_classes)
    state_dict = torch.load(ckpt_name)
    is_restore = True if 'cityscapes_best.pth' in ckpt_name else False
    model = load_model(model, state_dict, is_restore=is_restore, ddp=args.ddp)
    return model
...
def main(gpu, ngpus_per_node, config, args):
    ...
    cs_synth = CityscapesSynth(root=config.cs_synth_ood_root_path, transform=transform)
    acdc_synth = ACDCSynth(root=config.acdc_synth_ood_root_path, transform=transform)
    idd_synth = IDDSynth(root=config.idd_synth_ood_root_path, transform=transform)
    ...
    anomaly_save_path = f'/scratch/1/project/uss/anomaly_scores/RPL/{model_name}/'
    ...
    valid_anomaly(model=model, engine=engine, iteration=0, test_set=cs_synth, data_name='cs_synth',
                  my_wandb=vis_tool, logger=logger, measure_way=config.measure_way,
                  anomaly_save_path=anomaly_save_path)
    
    valid_anomaly(model=model, engine=engine, iteration=0, test_set=idd_synth, data_name='idd_synth',
                  my_wandb=vis_tool, logger=logger, measure_way=config.measure_way,
                  anomaly_save_path=anomaly_save_path)
    
    valid_anomaly(model=model, engine=engine, iteration=0, test_set=acdc_synth, data_name='acdc_synth',
                  my_wandb=vis_tool, logger=logger, measure_way=config.measure_way,
                  anomaly_save_path=anomaly_save_path)
...
if __name__ == '__main__':
    ...
    if args.model is not None:
        config.rpl_corocl_weight_path = args.model
