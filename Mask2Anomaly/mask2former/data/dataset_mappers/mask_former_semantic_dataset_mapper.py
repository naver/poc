    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_ood_ft: 
            if np.random.uniform() < self.anomaly_mix_ratio:
                path = random.choice(self.gt_list)
                if 'cs_synth' not in path:
                    coco_gt_path = path
                    ...
                    image =  utils.read_image(path, format=self.img_format).copy()
                    sem_seg_gt = utils.read_image(path.replace('/leftImg8bit/', '/gtFine/')).copy().astype("double")
                    sem_seg_gt[(sem_seg_gt > 18) & (sem_seg_gt != 255)] = 254 # Convert all OOD classes to 254 label
        
        # from PIL import Image
        # name = str(np.random.randint(1, 10000000))
        # Image.fromarray(image.astype('uint8')).save(f'/scratch/1/project/uss/test_img_{name}.png')
        # Image.fromarray(sem_seg_gt.astype('uint8')).save(f'/scratch/1/project/uss/test_label_{name}.png')
        # print(f'saving file: /scratch/1/project/uss/test_label_{name}')
